...
public class test {
    ...
    public void dijkstra_GetMinDistances(int sourceVertex){
        ...
        for (int i = 0; i <vertices ; i++) { // O(V)
            minHeap.insert(heapNodes[i]);//(logv)
        } 
        //while minHeap is not empty
        while(!minHeap.isEmpty()){ // O(V)
            //extract the min
            HeapNode extractedNode = minHeap.extractMin(); // (logv)
            ...
            LinkedList<Edge> list = adjacencylist[extractedVertex]; // O(e)
            for (int i = 0; i <list.size() ; i++) {
                ...
                if(currentKey>newKey){
                    decreaseKey(minHeap, newKey, destination); // O(logV)
    ...
    static class MinHeap{
        ...
        public void bubbleUp(int pos) { // O(logv)
            int parentIdx = pos/2;
            int currentIdx = pos;
